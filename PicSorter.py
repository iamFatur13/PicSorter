...
class PhotoSorter:
    def __init__(self, root):
        self.root = root
        self.root.title("Photo Sorter Pro - Stable Build")
        self.root.geometry("1100x850")
        
        # Grid Configuration (Baris 1 untuk gambar dibuat fleksibel)
        ...
        self.current_ctk_img = None  # Penjaga referensi agar tidak 'Image doesn't exist'
        
        self.setup_ui()
        
        # Bind Keyboard (1-4 untuk sortir, Z untuk undo)
    ...
    def setup_ui(self):
        ...
        self.load_btn = ctk.CTkButton(self.top_frame, text="📁 BUKA FOLDER FOTO", 
                                     command=self.load_folder, width=180, height=40,
                                     font=("Arial", 13, "bold"))
        self.load_btn.pack(side="left", padx=15, pady=10)

        self.info_label = ctk.CTkLabel(self.top_frame, text="Silahkan pilih folder...", 
                                      font=("Arial", 15, "bold"), text_color="#3498db")
        self.info_label.pack(side="right", padx=20)

        # --- 2. DISPLAY AREA (Row 1) ---
        self.img_container = ctk.CTkFrame(self.root, fg_color="#121212", corner_radius=10)
        self.img_container.grid(row=1, column=0, sticky="nsew", padx=20, pady=15)
        
        self.image_display = ctk.CTkLabel(self.img_container, text="Aplikasi Siap")
        self.image_display.pack(expand=True, fill="both", padx=10, pady=10)

        # --- 3. FOOTER CONTROLS (Row 2 & 3) ---
        self.filename_label = ctk.CTkLabel(self.root, text="-", font=("Consolas", 13))
        ...
        for i, opt in enumerate(self.options, 1):
            btn = ctk.CTkButton(self.control_frame, text=f"{opt}\n(Tekan {i})", 
                                fg_color=colors[opt], hover_color="#333",
                                text_color="white", font=("Arial", 14, "bold"), 
                                width=180, height=60,
                                command=lambda o=opt: self.sort_photo(o))
            btn.grid(row=0, column=i-1, padx=12)
    ...
    def load_folder(self):
        ...
        self.photo_list = [f for f in os.listdir(selected_path) 
                          if f.lower().endswith(ext) and os.path.isfile(os.path.join(selected_path, f))]
        self.photo_list.sort()
        
        if not self.photo_list:
            messagebox.showwarning("Kosong", "Folder ini tidak berisi file gambar yang didukung.")
            return
    ...
    def show_photo(self):
        """Memuat gambar ke UI dengan pengamanan memori."""
        if 0 <= self.current_index < len(self.photo_list):
            img_path = os.path.join(self.source_dir, self.photo_list[self.current_index])
            
            # Update Teks Info
            self.info_label.configure(text=f"PROGRESS: {self.current_index + 1} / {len(self.photo_list)}")
            self.filename_label.configure(text=f"📄 {self.photo_list[self.current_index]}")

            try:
                # 'with' memastikan file ditutup segera setelah dibaca
                with Image.open(img_path) as img:
                    img_view = img.copy() # Salin ke RAM
                    
                    # Update ukuran container secara real-time
                    self.root.update_idletasks()
                    cw = self.img_container.winfo_width() - 40
                    ch = self.img_container.winfo_height() - 40
                    if cw < 100: cw, ch = 800, 500
                    
                    img_view.thumbnail((cw, ch))
                    
                    # Simpan ke variabel class agar tidak terkena Garbage Collection
                    self.current_ctk_img = ctk.CTkImage(
                        light_image=img_view, 
                        dark_image=img_view, 
                        size=(img_view.width, img_view.height)
                    )
                    
                    self.image_display.configure(image=self.current_ctk_img, text="")
            except Exception as e:
                self.image_display.configure(image=None, text=f"Error Memuat Gambar:\n{e}")
        else:
            self.image_display.configure(image=None, text="SELESAI!\nSemua foto di folder ini sudah disortir.")
            self.filename_label.configure(text="Selesai")

    def sort_photo(self, category):
        """Proses pemindahan file ke subfolder."""
        ...
        # 1. Lepas gambar dari UI agar Windows tidak mengunci file (PENTING)
        ...
        try:
            ...
            # 5. Tampilkan Foto Berikutnya
            self.show_photo()
        except Exception as e:
            messagebox.showerror("Akses Ditolak", f"Gagal memindahkan file.\nPastikan file tidak sedang dibuka program lain.\n\nDetail: {e}")
            self.show_photo()

    def undo_action(self):
        """Membatalkan aksi terakhir (Shortcut: Z)."""
        ...
        try:
            ...
            # Kembalikan file ke daftar antrean
            self.photo_list.insert(self.current_index, os.path.basename(last_src))
            self.show_photo()
        except Exception as e:
            messagebox.showerror("Error Undo", f"Tidak bisa mengembalikan file: {e}")
